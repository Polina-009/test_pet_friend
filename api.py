import requests
import json
"""для теста test_unsuccessful_get_all_pets_with_invalid_key в методе get_list_of_pets вместо key подставляем 
следующие данные - 'ea738148a1f19838e1c5d1413877f3691a3731380e733e877b0ae729'"""
class PetFriends:
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Метод отправляет запрос на сервер об обновлении фотографии питомца по указанному ID и
        возвращает статус запроса и результат в формате JSON с обновлённым фото питомца"""
        headers = {'auth_key': auth_key['key']}
        file = {'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')}
